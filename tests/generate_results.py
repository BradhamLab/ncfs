...
for i, n in enumerate(n_features):
    ...
    axes[0, i] = plot_features(feature_select.coef_, ax=axes[0, i],
                               title='Manhattan distance, N = {}'.format(n))
    feature_select = NCFS.NCFS(eta=0.01)
    feature_select.fit(X, y, metric='euclidean')
    axes[1, i] = plot_features(feature_select.coef_, ax=axes[1, i],
...
for i, reg in enumerate(lambdas):
    ...
    feature_select.fit(X, y)
    axes[i] = plot_features(feature_select.coef_, ax=axes[i],
                            title='$\lambda = {}$'.format(reg))
