...
class SqeuclideanTest(unittest.TestCase):
    ...
    def test_sqeuclidean_pdist(self):
        X_dist = np.array([[0, 27, 108],
                           [27, 0, 27],
                           [108, 27, 0]])
        dist = np.zeros((3, 3))
        distances.pdist(self.X, self.w_, dist, distances.sqeuclidean,
                        symmetric=False)
        np.testing.assert_allclose(X_dist, dist)

    def test_sqeuclidean_partials0(self):
        partial = distances.SqEuclidean(self.X, self.w_)
        expected = np.array([[0, 18, 72],
                             [18, 0, 18],
                             [72, 18, 0]])
        D = np.zeros((3, 3))
        partial.partials(self.X, D, 0)
...
class EuclideanTest(unittest.TestCase):
    ...
    def test_euclidean_pdist(self):
        X_dist = np.array([[0, 5.196152422706632, 10.392304845413264],
                           [5.196152422706632, 0, 5.196152422706632],
                           [10.392304845413264, 5.196152422706632, 0]])
        dist = np.zeros((3, 3))
        distances.pdist(self.X, self.w_, dist, distances.euclidean,
                        symmetric=False)
        np.testing.assert_allclose(X_dist, dist)
...
class PhiSTest(unittest.TestCase):
    ...
    def test_phis_zero_weights(self):
        ...
        self.assertTrue(np.isnan(dist),
                         msg='{} not nan.'.format(dist))

    def test_pdist(self):
        A_dist = np.array([[0, 9, 1],
                           [9, 0, 1],
                           [1, 1, 0]])
        dist = np.zeros((3, 3))
